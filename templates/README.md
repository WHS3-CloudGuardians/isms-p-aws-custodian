...
본 문서는 우리 조직이 프라울러(Prowler)로 점검한 AWS 보안 항목을 실제로 조치할 수 있도록 작성한 Cloud Custodian 정책을 정리한 자료입니다.
즉, 프라울러가 탐지하는 보안 취약점과 컴플라이언스 미준수 항목을 단순히 “발견”하는 것에서 그치지 않고,
Cloud Custodian을 통해 자동 탐지·알림·조치가 가능하도록 정책을 수립하였습니다.
...
### 알림 색상
- 🟢 **조치완료 (good)**: 자동조치가 수반되어 위험이 위반사항이 조치됨
- 🟠 **경고 (warning)**: 운영 영향/합의 필요 또는 비실질 액션 중심(알림만)
- 🔴 **위험 (danger)**: 외부 노출/권한승격/암호화 미적용 등 고위험 상태

---

아래 표는 프라울러로 탐지 가능한 AWS 보안 점검 항목을 Cloud Custodian 정책으로 대응한 매트릭스입니다.
표는 영역별로 구분되어 있으며, 각 정책은 번호(영역-순번)로 식별됩니다.

각 행은 특정 AWS 서비스에 대한 보안 정책을 나타내며, 정책명, 설명, 리소스 유형, 권장 모드(CloudTrail/Periodic), 권장 조치 수준(알림/자동조치), 알림 색상, 위험도, 조치기간, 트리거 조건, 참조 템플릿 파일을 포함합니다.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 1-01 | accessanalyzer | accessanalyzer_enabled | Access Analyzer가 계정에<br>활성화되어 있는지 점검 | aws.account | periodic | notify-only | 🟠 경고 (warning) | 중 | 중기 | 주기 점검 시 정책 조건 위반 | accessanalyzer.yaml.template |
